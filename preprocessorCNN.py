...
data_path_pos = "D:\\datasets\\NLPLab\\data\\aclImdb_v1\\aclImdb\\train\\pos\\"
...
with tf.device('/device:GPU:0'):
    for pos_file, neg_file in zip(pos_files, neg_files):
        if counter >= 1900: #where the process left off
            with open(data_path_pos + pos_file, 'r', encoding='utf8') as pf:
                ptext = pf.read()
            with open(data_path_neg + neg_file, 'r', encoding='utf8') as nf:
                ntext = nf.read()
